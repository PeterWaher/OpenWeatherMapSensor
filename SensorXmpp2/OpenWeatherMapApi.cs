		...
		public static async Task<Field[]> GetData(string ApiKey, string Location, string Country)
		{
			...
			if (Response.TryGetValue("coord", out Obj) && Obj is Dictionary<string, object> Coord)
			{
				if (Coord.TryGetValue("lon", out Obj) && Obj is double Lon)
					Result.Add(new QuantityField(ThingReference.Empty, Timestamp, "Longitude", Lon, 2, "°", FieldType.Identity, FieldQoS.AutomaticReadout));

				if (Coord.TryGetValue("lat", out Obj) && Obj is double Lat)
					Result.Add(new QuantityField(ThingReference.Empty, Timestamp, "Latitude", Lat, 2, "°", FieldType.Identity, FieldQoS.AutomaticReadout));
			}
			...
			if (Response.TryGetValue("main", out Obj) && Obj is Dictionary<string, object> Main)
			{
				if (Main.TryGetValue("temp", out Obj) && Obj is double Temp)
					Result.Add(new QuantityField(ThingReference.Empty, Timestamp, "Temperature", Temp, 2, "°C", FieldType.Momentary, FieldQoS.AutomaticReadout));

				if (Main.TryGetValue("feels_like", out Obj) && Obj is double FeelsLike)
					Result.Add(new QuantityField(ThingReference.Empty, Timestamp, "Feels Like", FeelsLike, 2, "°C", FieldType.Computed, FieldQoS.AutomaticReadout));

				if (Main.TryGetValue("temp_min", out Obj) && Obj is double TempMin)
					Result.Add(new QuantityField(ThingReference.Empty, Timestamp, "Temperature, Min", TempMin, 2, "°C", FieldType.Peak, FieldQoS.AutomaticReadout));
				...
				WeatherArray.GetValue(0) is Dictionary<string, object> Weather)
			{
				...
				if (Weather.TryGetValue("description", out Obj) && Obj is string Description)
					Result.Add(new StringField(ThingReference.Empty, Timestamp, "Weather, Description", Description, FieldType.Momentary, FieldQoS.AutomaticReadout));

				if (Weather.TryGetValue("icon", out Obj) && Obj is string Icon)
					Result.Add(new StringField(ThingReference.Empty, Timestamp, "Weather, Icon", "http://openweathermap.org/img/wn/" + Icon + "@2x.png", FieldType.Momentary, FieldQoS.AutomaticReadout));
			}
